...
## Identidad y contexto exclusivos de Codex

Para Codex, las referencias a **Base Django React Next Feature** dentro de
`project-shared` describen únicamente el scaffold heredado, no la identidad del
producto actual.

- **Nombre**: Aviation Weather Viewer.
- **Propósito**: MVP demostrativo de un visor meteorológico aeronáutico para
  Aerocivil, con datos simulados y sin autorización para uso operacional.
- **Fuente funcional vigente**: `docs/MVP_roadmap/mvp_roadmap.md`.
- **Estado actual**: scaffold sin dominio, servicios ni despliegue activo.
- **Arquitectura objetivo**: Django/DRF, React + TypeScript, PostgreSQL/PostGIS,
  MapLibre GL JS y WebGL.

Este bloque reemplaza sólo la identidad de producto obsoleta del template. Las
reglas operativas, de seguridad, Git y testing del resto de `AGENTS.md` siguen
vigentes.
<!-- codex-specific:end -->
