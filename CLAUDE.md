...
# CLAUDE.md — Aviation Weather Viewer (`aviation_weather_viewer_project`)
...
# Aviation Weather Viewer — guía compartida del proyecto

## Identidad

- **Producto visible:** Meteorología Aeronáutica · Demo ProjectApp.
- **Servicio backend:** `aero-meteo-mvp`.
- **Propósito:** MVP demostrativo de un visor meteorológico aeronáutico para
  Colombia, con datos simulados y sin autorización para uso operacional.
- **Estado:** Fase 00; base técnica limpia y composición visual congelada.
- **Despliegue:** no existe servicio productivo activo durante esta fase.

## Fuentes de verdad

Para el MVP, resolver decisiones en este orden:

1. `docs/MVP_roadmap/phase_scopes/README.md`.
2. `docs/MVP_roadmap/phase_scopes/00_shared_contracts.md`.
3. El scope de fase correspondiente bajo `docs/MVP_roadmap/phase_scopes/`.
4. `docs/MVP_roadmap/mvp_roadmap.md`.

No modificar el roadmap ni los scopes como parte de una implementación de fase.

## Arquitectura vigente

- Backend: Python 3.12, Django 6, DRF y una única app de dominio, `weather`.
- Persistencia: PostgreSQL 16 + PostGIS; no existe fallback SQLite o MySQL.
- Frontend: Next.js 16, React 19, TypeScript, Tailwind CSS y Zustand.
- Cartografía objetivo: MapLibre GL JS/WebGL, instalado pero no inicializado en
  Fase 00.
- HTTP público y same-origin bajo `/api/v1/`, sin JWT, login ni dominio de
  usuarios.

No reintroducir comercio, catálogo, carrito, checkout, blog, backoffice,
dashboard, manual, OAuth, CAPTCHA, attachments, Redis, Huey, themes configurables
ni staging gates.

## Seguridad y configuración

- Nunca versionar secretos, tokens, DSN, dumps ni archivos `.env`.
- Documentar variables solo con placeholders en `.env.example`.
- Django debe fallar con un mensaje accionable si falta configuración Postgres.
- No exponer credenciales, hosts, usuarios ni paths internos en respuestas HTTP.
- El warning de datos simulados es permanente y no puede ocultarse.

## Estructura principal

```text
backend/aviation_weather_project/  configuración Django
backend/weather/                   dominio y API meteorológica
frontend/app/                      App Router y visor
frontend/e2e/                      registro y pruebas de flujos reales
docs/methodology/                  Memory Bank técnico
tasks/                             contexto y plan activo
```

## Testing

- Nunca ejecutar la suite completa: usar siempre archivos dirigidos.
- Máximo 20 tests por batch y 3 comandos por ciclo.
- Backend: `cd backend && source venv/bin/activate && pytest <archivo> -v`.
- Frontend: `cd frontend && npm test -- <archivo>`.
- E2E: máximo 2 specs por invocación.
- Ejecutar `npm run build` en un ciclo separado de los tests.
- Cada test comprueba un comportamiento observable, sigue AAA y no contiene
  condicionales.

## Memory Bank

Los siete archivos obligatorios son:

1. `docs/methodology/product_requirement_docs.md`
2. `docs/methodology/architecture.md`
3. `docs/methodology/technical.md`
...
Leerlos antes de cambios significativos y actualizar los siete cuando cambien
identidad, arquitectura, apps, dependencias o estado de una fase.
